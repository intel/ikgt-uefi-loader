...
int elf_is_elf(const char *p_buffer)
{
	...
	return ELFMAG0 == p_buffer[EI_MAG0] &&
	       ...
	       ELFMAG2 == p_buffer[EI_MAG2] && ELFMAG3 == p_buffer[EI_MAG3];
}
...
int elf32_header_is_valid(const void *p_buffer)
{
	...
	return ELFCLASS32 == ehdr->e_ident[EI_CLASS] &&
	       ...
	       EV_CURRENT == ehdr->e_version && EM_386 == ehdr->e_machine;
}
...
int elf64_header_is_valid(const void *p_buffer)
{
	...
	return ELFCLASS64 == ehdr->e_ident[EI_CLASS] &&
	       ...
	       ELFMAG3 == ehdr->e_ident[EI_MAG3] &&
	       ELFDATA2LSB == ehdr->e_ident[EI_DATA] &&
	       EV_CURRENT == ehdr->e_version && EM_X86_64 == ehdr->e_machine;
}

int
elf_get_segment_info(const void *p_image,
		     int16_t segment_no, elf_segment_info_t *p_info)
{
	if (TRUE == elf32_header_is_valid(p_image)) {
		return elf32_get_segment_info((elf32_ehdr_t *)p_image, segment_no,
			p_info);
	} else if (TRUE == elf64_header_is_valid(p_image)) {
		return elf64_get_segment_info((elf64_ehdr_t *)p_image, segment_no,
			p_info);
	} else {
		return FALSE;
	}
...
int
elf32_get_segment_info(elf32_ehdr_t *ehdr,
		       int16_t segment_no, elf_segment_info_t *p_info)
{
...
int
elf64_get_segment_info(elf64_ehdr_t *ehdr,
		       int16_t segment_no, elf_segment_info_t *p_info)
{
	if (segment_no < ehdr->e_phnum) {
		uint8_t *phdrtab = (uint8_t *)ehdr + ehdr->e_phoff;
